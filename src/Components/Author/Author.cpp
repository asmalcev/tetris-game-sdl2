    ...
    box({x, y, w, h})
{
  ...
  SDL_FreeSurface(textSurf);
  SDL_DestroyTexture(AuthorTexture);
  textSurf = renderText("The figure's falling speed increases", {245, 245, 245}, 16, "docs/RobotoMono-Regular.ttf", &tw, &th);
  textPos = { (box.w - tw) / 2, 170, tw, th};
  AuthorTexture = SDL_CreateTextureFromSurface(renderer, textSurf);
  SDL_RenderCopy(renderer, AuthorTexture, NULL, &textPos);
  SDL_FreeSurface(textSurf);
  SDL_DestroyTexture(AuthorTexture);
  textSurf = renderText("as you score 1000 points and 2000 points", {245, 245, 245}, 16, "docs/RobotoMono-Regular.ttf", &tw, &th);
